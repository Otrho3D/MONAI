...
def to_affine_nd(r: Union[np.ndarray, int], affine: np.ndarray, dtype=np.float64) -> np.ndarray:
    """
    ...
    """
    affine_np = np.array(affine, dtype=dtype, copy=True)
    if affine_np.ndim != 2:
...
def ensure_mat44(affine, dtype=np.float64) -> np.ndarray:
    ...
    affine, *_ = convert_data_type(affine, output_type=np.ndarray, dtype=dtype, wrap_sequence=True)
    if affine.shape == (4, 4):
        return affine.astype(dtype, copy=True)
    return to_affine_nd(r=3, affine=affine, dype=dtype)
